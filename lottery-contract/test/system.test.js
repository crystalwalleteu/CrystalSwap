...
describe("Lottery contract", function() {
    ...
    // Creating the instance and contract info for the crystal token contract
    let crystalInstance;
    ...
    beforeEach(async () => {
        ...
        crystalInstance = await mock_erc20Contract.deploy(
            lotto.buy.crystal,
        );
        ...
        // Making sure the lottery has some crystal
        await crystalInstance.mint(
            lotteryInstance.address,
    ...
    describe("Buying tickets tests", function() {
        ...
        it("Batch buying 1 tickets", async function() {
            ...
            await crystalInstance.connect(owner).approve(
                lotteryInstance.address,
                price
            );
            // Batch buying tokens
         ...
         */
        it("Invalid crystal transfer", async function() {
            // Getting the price to buy
            let price = await lotteryInstance.costToBuyTickets(
                1,
                10
    ...
    describe("Claiming tickets tests", function() {
        beforeEach( async () => {
            ...
            // Sending the buyer the needed amount of crystal
            await crystalInstance.connect(buyer).mint(
                buyer.address,
                prices[2]
            );
            // Approving lotto to spend cost
            await crystalInstance.connect(buyer).approve(
                lotteryInstance.address,
                prices[2]
            );
            // Generating chosen numbers for buy
        ...
        it("Claiming winning numbers (4 (all) match)", async function() {
            ...
            // Sending the buyer the needed amount of crystal
            await crystalInstance.connect(owner).transfer(
                buyer.address,
                prices[2]
            );
            // Approving lotto to spend cost
            ...
            let buyerCakeBalanceBefore = await crystalInstance.balanceOf(buyer.address);
            ...
            let buyerCakeBalanceAfter = await crystalInstance.balanceOf(buyer.address);
            // Tests
            assert.equal(
                buyerCakeBalanceBefore.toString(),
                0,
                "Buyer has crystal balance before claiming"
            );
            assert.equal(
    ...
    describe("Upgrade functionality tests", function() {
        ...
        it("Withdraw excess crystal", async function() {
            let ownerCakeBalance = await crystalInstance.balanceOf(owner.address);
            let lotteryCakeBalance = await crystalInstance.balanceOf(lotteryInstance.address);
            await lotteryInstance.connect(owner).withdrawCake(lotteryCakeBalance);
            let ownerCakeBalanceAfter = await crystalInstance.balanceOf(owner.address);
            let lotteryCakeBalanceAfter = await crystalInstance.balanceOf(lotteryInstance.address);
            // Tests
            assert.notEqual(
                ownerCakeBalance.toString(),
                ownerCakeBalanceAfter.toString(),
                "Owner crystal balance did not change"
            );
            assert.notEqual(
        ...
        it("Invalid withdraw excess crystal (non owner)", async function() {
            let lotteryCakeBalance = await crystalInstance.balanceOf(lotteryInstance.address);
            await expect(
