...
contract('Timelock', ([alice, bob, carol, dev, minter]) => {
    beforeEach(async () => {
        this.crystal = await CakeToken.new({ from: alice });
        this.timelock = await Timelock.new(bob, '28800', { from: alice }); //8hours
    });

    it('should not allow non-owner to do operation', async () => {
        await this.crystal.transferOwnership(this.timelock.address, { from: alice });
        await expectRevert(
            this.crystal.transferOwnership(carol, { from: alice }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.crystal.transferOwnership(carol, { from: bob }),
            'Ownable: caller is not the owner',
        );
        await expectRevert(
            this.timelock.queueTransaction(
                this.crystal.address, '0', 'transferOwnership(address)',
    ...
    it('should do the timelock thing', async () => {
        ...
        assert.equal((await this.crystal.owner()).valueOf(), carol);
    });
    ...
    it('should also work with MasterChef', async () => {
        ...
        this.chef = await MasterChef.new(this.crystal.address, this.syrup.address, dev, '1000', '0', { from: alice });
        await this.crystal.transferOwnership(this.chef.address, { from: alice });
