...
contract('SyrupBar', ([alice, bob, carol, dev, minter]) => {
  ...
  it('safeCakeTransfer', async () => {
    assert.equal(
      (await this.crystal.balanceOf(this.syrup.address)).toString(),
      '0'
    );
    await this.crystal.mint(this.syrup.address, 1000, { from: minter });
    await this.syrup.safeCakeTransfer(bob, 200, { from: minter });
    assert.equal((await this.crystal.balanceOf(bob)).toString(), '200');
    assert.equal(
      (await this.crystal.balanceOf(this.syrup.address)).toString(),
      '800'
    );
    await this.syrup.safeCakeTransfer(bob, 2000, { from: minter });
    assert.equal((await this.crystal.balanceOf(bob)).toString(), '1000');
  });
});
