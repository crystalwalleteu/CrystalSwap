...
import currentCrystalswapDefaultList from "../lists/crystalswap-default.json";
import currentCrystalswapExtendedtList from "../lists/crystalswap-extended.json";
import currentCrystalswapTop15List from "../lists/crystalswap-top-15.json";
import currentCrystalswapTop100tList from "../lists/crystalswap-top-100.json";
import { buildList, VersionBump } from "../src/buildList";

const currentLists = {
  "crystalswap-default": currentCrystalswapDefaultList,
  "crystalswap-extended": currentCrystalswapExtendedtList,
  "crystalswap-top-100": currentCrystalswapTop100tList,
  "crystalswap-top-15": currentCrystalswapTop15List,
};
...
expect.extend({
  ...
  toBeValidLogo(token) {
    ...
    const refersToLocalLogo =
      token.logoURI === `https://tokens.crystalswap.finance/images/${token.address}.png` ||
      token.logoURI === `https://tokens.crystalswap.finance/images/${token.address.toLowerCase()}.png`;
    if (token.logoURI === "https://tokens.crystalswap.finance/images/0x4e6415a5727ea08aae4580057187923aec331227.png") {
...
describe.each([["crystalswap-default"], ["crystalswap-extended"], ["crystalswap-top-100"], ["crystalswap-top-15"]])(
  "buildList %s",
  (listName) => {
