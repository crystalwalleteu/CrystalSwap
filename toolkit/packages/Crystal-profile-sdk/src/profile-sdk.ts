...
class CrystalProfileSdk {
   ...
   * API repo - https://github.com/crystalswap/crystal-profile-api
   */
   ...
   * Contract repo - https://github.com/crystalswap/crystal-contracts/tree/master/projects/profile-nft-gamification
   */
   ...
   * from CrystalProfile contract.
   * NFT's bunnyId is retrieved from CrystalBunnies contract and mapped to static NFT data stored in constant.
   * Contracts repo - https://github.com/crystalswap/crystal-contracts/tree/master/projects/profile-nft-gamification
   */
  getProfile = async (address: string): Promise<GetProfileResponse> => {
    try {
      ...
      if (isActive) {
        ...
        const avatar = nft ? `https://crystalswap.finance/images/nfts/${nft.images.sm}` : undefined;
        ...
        Cookies.set(
          ...
          { domain: "crystalswap.finance", secure: true, expires: 30 }
        );
      }
...
export default CrystalProfileSdk;
