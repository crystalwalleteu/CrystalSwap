# Crystalswap Profile SDK

This package provides some handy functions to retrieve data for Crystalswap Profile system.
...
Install `@crystalswap/profile-sdk` into your project with npm:

```bash
npm install @crystalswap/profile-sdk --save
```

or yarn:

```bash
yarn add @crystalswap/profile-sdk
```
...
```js
import CrystalProfileSdk from "@crystalswap/profile-sdk";

const crystalSdk = new CrystalProfileSdk();
```
...
const crystalSdk = new CrystalProfileSdk(customProvider, 97);
```
...
const crystalSdk = new CrystalProfileSdk();
const username = crystalSdk.getUsername("0x123456789");
...
const crystalSdk = new CrystalProfileSdk();
const team = crystalSdk.getTeam(1);
console.log(team);
// {
//   id: 1,
//   name: "Syrup Storm",
...
It also sets `profile_${address}` cookie containing username and avatar (now only for crystalswap.finance domain, maybe configurable in future versions)

```js
import CrystalProfileSdk from "@crystalswap/profile-sdk";

const crystalSdk = new CrystalProfileSdk();
const profile = crystalSdk.getProfile("0x123456789");
console.log(profile);
// {
//   hasRegistered: true
//   profile: {
...
const achievements = crystalSdk.getAchievements("0x123456789");
...
```JSX
import { Team, teamImages } from "@crystalswap/profile-sdk";
...
Current version of this SDK is 90% copy of existing from [crystal-frontend](https://github.com/crystalswap/crystal-frontend) repo. There are several improvements to be made in the future versions of this SDK:
